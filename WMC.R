# Nonparametric pairwise multiple comparisons using the Wilcoxon
# Signed Rank Test
# Probability values are adjusted using the p.adjust function
wmc <- function(formula, data, exact=FALSE, sort=TRUE,
		method="holm"){
  ...
  # summary statistics
  stats <- function(z)(c(N = length(z), Median = median(z), MAD =
mad(z)))
